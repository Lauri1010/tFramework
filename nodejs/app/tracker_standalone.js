(function(window,document){
	...
	if(jQuery) {
		...
		/***
		Tracker
		trackPageview: tracking pageview on or off
		mode: for conversion tracking: what is the mode, o= prevent default with continue, 1= standard
		tm: What tracking method is beign used. 1= google 
		cm: errors ad other important notifications are sent to console
		*/	
		...
		var tr=function(){
			...
			this.$r = function(opts){
			    ...
			    if(opts.url && (opts.m===1 || opts.m===2)){
			        ...
			        r.src = this.protocol+opts.url + '?' + qString;
			        if(opts.m===2){
		...
		// true, 1,1,true
		tr.prototype.i=function(ttid,cm,fd,pageName,action,bust){
				var self=this;
				if(typeof ttid === 'string'){self.ttid=ttid};
				...
				&& (bust == true || bust == false)){
					...
					if(!self.aOptOut){
						...
						if(!self.rc('tt_vid')){
							self.cc('tt_vid',ts,1800,self.chn);
						}
						
						if(!self.rc('tt_uvid')){
							self.cc('tt_uvid',ts,63113852,self.chn);
						}
						
						if (typeof(Storage) !== "undefined") {
						    
							if(!sessionStorage.getItem('tt.tap')){
								var tap={
									pc:1,
									tp:0,
									a:0
								};
								sessionStorage.setItem('tt.tap', JSON.stringify(tap));
							}
					...
					&& !self.aOptOut){
						if(pageName.length<self.pnMaxLenght){
							if(c=='hit'){
								...
								if(self.sTime && typeof self.sTime === 'number'){
									var sdiff=Math.floor(((new Date()).getTime()-self.sTime)/1000);
									self.cc('tti_time_c',sdiff,20,self.pDomain);
								}
								...
								self.$r({
							       ...
							       vars : p,
							       error : function(){
							    	   if(cm==true){console.log("error in request");}
							       },
							       success : function(c){
		...
		tr.prototype.ttime=function(ttid,tm,pageName,sTime,bust){
			var self=this;
			self.sTime=sTime;
			...
			$(function() {
                $(document).on("click touchstart", "a", function(e) {
                	...
                	var tta=$(this).attr("data-tta");
                	
                	if(self.cm){ 
						console.log('tta '+tta);
					}

					if(tta && typeof tta === 'string' && tta=="true"
						&& self.sTime && typeof self.sTime === 'number' 
						&& ttid && typeof ttid === 'string' 
						&& (bust === true || bust === false)
						&& (tm===1 || tm===2)){
						...
						var sdiff=Math.floor((((new Date()).getTime())-self.sTime)/1000);
						self.cc('tt_time_c',sdiff,20,self.pDomain);
